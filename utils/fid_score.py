...
def calculate_fid_given_images(images0, images1, batch_size=20, cuda=True, dims=2048):
    ...
    images0 = images0.mul(0.5).add(0.5).cpu().data.numpy()
    images1 = images1.mul(0.5).add(0.5).cpu().data.numpy()

    m1, s1 = _compute_statistics_of_path(images0, model, batch_size,
                                         dims, cuda)
    m2, s2 = _compute_statistics_of_path(images1, model, batch_size,
                                         dims, cuda)
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)

    return fid_value

# def cal_fid_score(fake_images, images):
#     # generator data
#     generator_datas = fake_images.cpu().data.numpy()
#     # original data
#     # _, data_loader = get_cifar_dataloaders(batch_size=100)
#     # data_iter = iter(data_loader)
#     # original_datas, __ = next(data_iter)
#     original_datas = images.cpu().data.numpy()
#     # fid
